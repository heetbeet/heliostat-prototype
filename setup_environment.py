...
def install_winpython():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with tempfile.TemporaryDirectory() as extract_temp_dir:
            ...
            subprocess.run([str(s7ip_dir / "7z.exe"), "x", str(winpython_installer_path), f"-o{extract_temp_dir}"])
            ...
            for root, dirs, files in os.walk(extract_temp_dir):
                if 'WPy64' in root and any('python' in d and 'amd64' in d for d in dirs):
                    python_installation_dir = Path(root) / [d for d in dirs if 'python' in d and 'amd64' in d][0]
                    break
        ...
        requirements_file = parent_dir / 'requirements.txt'  # Assuming requirements.txt is in the same folder as the script
        subprocess.run([str(venv_dir / 'Scripts' / 'python.exe'), '-m', 'pip', 'install', '-r', str(requirements_file)])

def install_cygwin():
    # evaluate if already installed
    try:
        if "Usage: gcc" in subprocess.check_output([str(cygwin_dir / "cygwin-portable.cmd"), "-c", "gcc --help"], stderr=subprocess.STDOUT).decode('utf-8'):
    ...
    for i, line in enumerate(lines):
        if line.strip().startswith("echo Loading") or line.strip().startswith("echo Replacing"):
            lines[i] = ""
